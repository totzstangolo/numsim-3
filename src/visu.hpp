...
public:
	...
	void SetSlice (const index_t& xdim, const index_t& ydim, const multi_real_t& origin);
...
//------------------------------------------------------------------------------
/*! \class	Renderer
 *	This class renders grids on a SDL2 GUI using the grids evaluate function.
 ...
 *	Constructs a Renderer with a given mesh width \p h and total size \p length
 *	of the grid.
 *
 *	\param	h		mesh width to use
 *	\param	length	total size of the area
 */
/*!	\fn	void Renderer::Init (const index_t& width, const index_t& height, const int& idx = 0)
 *	Initializes the SDL window and sets its size.
 *
 *	\param	width	The width of the window in pixels
 *	\param	height	The height of the window in pixels
 *	\param	idx		The Index of the window. If zero, no index is printed in the title.
 */
/*!	\fn void Renderer::SetSlice (const index_t& xdim, const index_t& ydim, const multi_real_t& origin)
 *	Defines witch dimensions to display on the window with \p xdim and \p ydim.
 *	All other dimensions are evaluated at the positions defined in \p origin.
 *
 ...
 *		\arg	0~9: The current status of the window. It can be changed by pressing
 *					a number key.
 *		\arg	10: The enter key was pressed. This doesn't effect the status.
 */
 ...
 */
/*!	\fn int Renderer::Render (const Grid *grid, const multi_real_t& offset, const real_t& min, const real_t& max)
 *	Updates the SDL window and checks for pressed keys. The visualization is
 *	scaled based on \p min and \p max. Grid values are cropped by these limits.
 *
 *	\param grid		Pointer to the grid that should be visualized.
